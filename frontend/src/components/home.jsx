...
const getDestinationTitle = (destination) => destination.title || destination.name || "";

const getDestinationImage = (destination, fallbackImages = []) =>
  destination.mainImage || destination.cardImage || destination.image || fallbackImages[0] || "";

const getTripDetails = (destination, images) => ({
  ...destination,
  title: getDestinationTitle(destination),
  location: destination.region || destination.location,
  group: destination.maxPeople,
  language: destination.languages || "English",
  image: getDestinationImage(destination, images),
  source: "destination",
});

function Home({ destinations = [], images = [], onNavigate, onViewDetails }) {
  ...
  const destinationOptions = useMemo(() => Array.from(
    new Map(
      destinations
        ...
        .map((destination) => {
          ...
          return [destination._id || title, { ...destination, title }];
        })
    ).values()
  ), [destinations]);

  const trekDestinations = useMemo(
    () => destinations
      .filter((destination) => destination.status !== "inactive" && destination.destinationType === "trek")
      .map((destination) => ({ ...destination, title: getDestinationTitle(destination) })),
    [destinations],
  );
              ...
              {(filter === "Choose Trek" ? trekDestinations : destinationOptions).map((dest, index) => (
                <li 
                  ...
                  onClick={(e) => {
                    ...
                    if (onViewDetails) {
                      onViewDetails(getTripDetails(dest, images));
                      return;
                    }
