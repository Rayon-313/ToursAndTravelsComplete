...
const emptyDestination = {
  name: '', description: '', duration: '', maxPeople: '', languages: '',
  location: '', price: '', rating: '4.9',
...
export default function ManageDestinations() {
  ...
  useEffect(() => {
    ...
    setFiltered(destinations.filter((d) =>
      d.name?.toLowerCase().includes(q) || d.destinationType?.includes(q)
    ));
  }, [search, destinations]);
  ...
  const openEdit = (dest) => {
    setForm({
      name: dest.name || '', description: dest.description || '',
      duration: dest.duration || '', maxPeople: dest.maxPeople || '', languages: dest.languages || '',
      location: dest.location || '', price: dest.price || '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { ...form, title: form.name };
    try {
      if (editing) {
        await api.updateDestination(editing._id, payload, mainFile, cardFile);
        showToast('Destination updated!', 'success');
      } else {
        await api.createDestination(payload, mainFile, cardFile);
